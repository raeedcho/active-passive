...
for trial_num = 1:4
    ...
    plot(times,joint_vel,'linewidth',2)
    hold on
    % joint_vel = td_still(trial_num).opensim(:,opensim_vel_idx);
    % plot([times(1);times(end)],repmat(mean(joint_vel),2,1),'--','linewidth',2)
    yyaxis right
    plot(times,joint_mom,'linewidth',2)
    hold on
    % joint_mom = td_still(trial_num).opensim(:,opensim_mom_idx);
    % plot([times(1);times(end)],repmat(mean(joint_mom),2,1),'--','linewidth',2)
    ylims = get(gca,'ylim');
    plot(times(repmat(td_act(trial_num).idx_movement_on,2,1)),ylims,'k--','linewidth',2)
    ...
    % plot([times(1);times(end)],repmat(mean(joint_mom),2,1),'--','linewidth',2)
    ylims = get(gca,'ylim');
    plot(times(repmat(td_pas(trial_num).idx_bumpTime,2,1)),ylims,'k--','linewidth',2)
    ...
    joint_vel = td_act(trial_num).opensim(:,opensim_vel_idx);
    joint_mom = td_act(trial_num).opensim(:,opensim_mom_idx) - mom_offset;
    plot(times,joint_vel.*joint_mom,'linewidth',2)
    hold on
    ylims = get(gca,'ylim');
    plot(times(repmat(td_act(trial_num).idx_movement_on,2,1)),ylims,'k--','linewidth',2)
    ...
    plot(times,sum(joint_vel.*joint_mom,2),'linewidth',2)
    hold on
    ylims = get(gca,'ylim');
    plot(times(repmat(td_act(trial_num).idx_movement_on,2,1)),ylims,'k--','linewidth',2)
    ...
    times = -num_bins_before:length(td_pas(trial_num).opensim(:,opensim_vel_idx))-(num_bins_before+1);
    times = times*td_pas(trial_num).bin_size;
    %calculate velocity
